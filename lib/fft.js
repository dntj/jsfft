!function(exports, complex_array) {
  ...
  ComplexArray.prototype.frequencyMap = function(filterer) {
    return FFT(FFT(this, false).map(filterer), true)
  }
  ...
  function FFT_Recursive(input, inverse) {
    ...
    for(j = 0; j < p; j++) {
      ...
      recursive_result.map(function(value, index) {
        value.real = input.real[index * p + j]
        value.imag = input.imag[index * p + j]
      })
      ...
      output.map(function(value, i) {
        var
          _real = recursive_result.real[i % m],
          _imag = recursive_result.imag[i % m]

        value.real += normalisation * (f_r * _real - f_i * _imag)
        value.imag += normalisation * (f_r * _imag + f_i * _real)
