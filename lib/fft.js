!function(exports, complex_array) {
  ...
  function FFT_Recursive(input, inverse) {
    ...
    // for a power of a prime, p, this reduces to O(n log_p n)
    for(j = 0; j < p; j++) {
      for(i = 0; i < m; i++) {
      ...
      for(i = 0; i < n; i++) {
        ...
        output.real[i] += normalisation * (f_r * _real - f_i * _imag)
        output.imag[i] += normalisation * (f_r * _imag + f_i * _real)
  ...
  function BitReverseComplexArray(array) {
    ...
    for(i = 0; i < n; i++) {
      ...
      bitreversed_values.real[r_i] = array.real[i]
      bitreversed_values.imag[r_i] = array.imag[i]
    }
